...
export class PgWalletRepository implements IWalletRepository {
    ...
    async updateCashAccountWalletByUserId(userId: string, cash: number, account: number): Promise<Wallet> {
        return await prisma.wallet.update({
            where: { userId },
            data: {
                cash: BigInt(cash),
                account: BigInt(account),
            },
        });
    }
    ...
    //목표금액(target) 업데이트 메서드
    async updateTargetByUserId(userId: string, newTarget: number): Promise<Wallet> {
        return await prisma.wallet.update({
            where: { userId },
            data: { target: BigInt(newTarget) },
        });
    }
}
